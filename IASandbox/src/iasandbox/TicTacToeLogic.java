...
import iasandbox.TicTacToe.TicTacToeHuman;
import iasandbox.TicTacToe.TicTacToeMiniMax;
import iasandbox.TicTacToe.TicTacToeAlfaBeta;
import iasandbox.TicTacToe.TicTacToeRules;
import iasandbox.TicTacToe.ticTacToePlayer;
...
public class TicTacToeLogic {
 ...
 /*
        Methods
     */
    public static TicTacToeLogic getInstance() {
        return (instance == null ? instance = new TicTacToeLogic() : instance);
    }
    ...
    public void ticTacToe() {
        if (gameRunning) {
            if (!moveMade) {
                if (player1Turn) {
                    player1Turn = TicTacToeLogic.getInstance().player1Move();
                    player2Turn = !player1Turn;
                    moveMade = !player1Turn;
                } else if (player2Turn) {
                    player2Turn = TicTacToeLogic.getInstance().player2Move();
                    player1Turn = !player2Turn;
                    moveMade = !player2Turn;
                }
            }
            switch (TicTacToeLogic.getInstance().checkBoard(TicTacToeLogic.getInstance().getMap(), TicTacToeLogic.getInstance().getMovesMade())) {
                case (0): {
                    endGame();
    ...
    public void endGame() {
        gameRunning = false;
        if (TicTacToeLogic.getInstance().getPlayerClicked1() != null) {
            ControleUI.getInstance().getMainController().getCanvas().removeEventHandler(MouseEvent.MOUSE_CLICKED, TicTacToeLogic.getInstance().getPlayerClicked1());
        }
        if (TicTacToeLogic.getInstance().getPlayerClicked2() != null) {
            ControleUI.getInstance().getMainController().getCanvas().removeEventHandler(MouseEvent.MOUSE_CLICKED, TicTacToeLogic.getInstance().getPlayerClicked2());
        }
    }
    ...
    public void drawPlayerMovesTicTacToe() {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                switch (TicTacToeLogic.getInstance().getMap()[j][i]) {
                    case (1): {
